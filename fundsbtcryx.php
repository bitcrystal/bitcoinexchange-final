...
$cid = $my_coins->getCoinsSelectInstanceId()+2;
   ...
   if($withdraw_address) {
      if($withdraw_amount) {
         ...
         if($withdraw_amount<=$Bitcrystalxd_Balance[$iid]) {
            ...
            $Bitcrystalxd_Withdraw_From = $Bitcrystalxd[$iid]["daemon"]->sendtoaddress($withdraw_address,(float)$true_withdraw_amount);
            if($Bitcrystalxd_Withdraw_From) {
               ...
               $Bitcrystalxd_Balance[$iid] = userbalance($user_session,$my_coins->coins_names_prefix[$cid]);
               ...
               if(!mysql_query("INSERT INTO transactions (id,date,username,action,coin,address,txid,amount,trade_id) VALUES ('','$date','$user_session','withdraw','".$my_coins->coins_names_prefix[$cid]."','$withdraw_address','$Bitcrystalxd_Withdraw_From','$withdraw_amount','".$my_coins->coins_names_prefix[0+$cid]."_".$my_coins->coins_names_prefix[1+$cid]."_".$my_coins->coins_names_prefix[$cid]."')")){
            ...
            $withdraw_message = 'You do not have enough '.$my_coins->coins_names[$cid].'!';
         }
      } else {
