...
class w_coins {
	...
	public function set_coins_balance($name, $account, $account2, $amount)
	{
		...
		if($diff<0)
		{
			$daemon->sendtoaddress($daemon->getmaccountaddress($account2), abs($diff));
		...
		{
			$daemon->sendtoaddress($daemon->getmaccountaddress($account), $diff);
			$balance = $this->get_coins_balance($name, $account, 6);
			while($balance == false)
			{
				$balance = $this->get_coins_balance($name, $account, 6);
	...
	public function get_coins_address($name, $wallet_id)
	{
		...
		return $daemon->getmaccountaddress($wallet_id);
	}
